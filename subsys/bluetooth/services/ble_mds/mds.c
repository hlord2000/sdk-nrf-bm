...
{
	const ble_gatts_evt_rw_authorize_request_t *auth =
		&gatts_evt->params.authorize_request;
	ble_gatts_rw_authorize_reply_params_t reply = {
		.type = auth->type,
	};
	uint32_t nrf_err;
	uint16_t status = BLE_GATT_STATUS_ATTERR_ATTRIBUTE_NOT_FOUND;
...
static void log_collection_maybe_trigger(struct ble_mds *mds)
{
...
static bool pending_payload_prepare(struct ble_mds *mds)
{
	...
	log_collection_maybe_trigger(mds);
